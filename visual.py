#!/usr/bin/env python2

import sys
import pygame
import random

pygame.init()

window = pygame.display.set_mode((500,500))

points = {1:(82,76), 2: (96, 44), 3: (50, 5), 4: (49, 8), 5: (13, 7), 6: (29, 89), 7: (58, 30), 8: (84, 39), 9: (14, 24), 10: (2, 39), 11: (3, 82), 12: (5, 10), 13: (98, 52), 14: (84, 25), 15: (61, 59), 16: (1, 65), 17: (88, 51), 18: (91, 2), 19: (19, 32), 20: (93, 3), 21: (50, 93), 22: (98, 14), 23: (5, 42), 24: (42, 9), 25: (61, 62), 26: (9, 97), 27: (80, 55), 28: (57, 69), 29: (23, 15), 30: (20, 70), 31: (85, 60), 32: (98, 5)}

# ways = [[7,24,29,5,12,9,10,19,15],
#        [4,3,14,8,2,13],
#        [28,25],
#        [27,18,20,32,22,17,31],
#        [21,30,23,16,11,26,6]]

# ways = [[21,30,9,12,5,29,7,25],
#        [15,18,20,32,22,2,13],
#        [28,19,10,23,16,11,26,6],
#        [31,27,17,8,14,3,4,24]]

ways = [[24,3,21,10,16,11],
       [23,31,27,4,7,15,19,30,25],
       [8,18,5,32,20],
       [17,13,26,6],
       [29,9,12,14,28,2,22]]

ways = [[29,12,26,6,21],
       [19,9,3,32,2,15,17],
       [13,8,22,20,18],
       [4,5,25,14,24,7],
       [30,11,16,10,23,28,27,31]]

ways = [[20,32,18,3,24,29,15],
       [13,27,17,31,14,8,2],
       [28,6,16,11,30,26,25],
       [21,7,19,5,9,23,10,12,22,4]]

ways = [[17,13,18,20,32,27],
       [25],
       [21,6,30,11,16,19,9,29,22,2],
       [28,15,5,12,10,23,26],
       [31,8,14,3,4,24,7]]

ways = [[29,5,12,9,19,28],
       [25,15,7,24,4,3,8],
       [14,18,20,32,22,2],
       [30,23,10,16,11,26,6,21],
       [13,17,27,31]]

ways = [[25,15,7,14,8],
       [23,10,19,9,12,5,29,3],
       [24,4,18,20,32,22,2],
       [28,30,16,11,26,6,21],
       [31,27,17,13]]

for i, car in enumerate(ways):
    color = (random.randrange(255), random.randrange(255), random.randrange(255))
    prev = 1
    for city in car:
        x1, y1 = points[prev]
        x2, y2 = points[city]
        pygame.draw.line(window, color, (x1*5, y1*5), (x2*5, y2*5))
        prev = city
    x1, y1 = points[prev]
    x2, y2 = points[1]
    pygame.draw.line(window, color, (x1*5, y1*5), (x2*5, y2*5))

pygame.display.flip()

while True: 
   for event in pygame.event.get(): 
      if event.type == pygame.QUIT: 
          sys.exit(0) 
